...
for i=1:1
   ...
   x1=num2(h1,5);
   y1=num2(h1,6);
   [p1,s] = polyfit(x1,y1,1);
   R2= 1 - (s.normr/norm(y1 - mean(y1)))^2;
   
   Pper(i,1:2)=p1;
   Pper(i,3)=R2;
 
   h2=h(find(ismember(condlist(h),Stim)>0));
   x2=num2(h2,5);
   y2=num2(h2,6);
   
   [p2,s] = polyfit(x2,y2,1);
   R2= 1 - (s.normr/norm(y2 - mean(y2)))^2;
   ...
   Y=[y1;y2]/100000;
   X=[x1;x2];
   X1=[ones(length(x1),1); zeros(length(x2),1)];
   X2=[zeros(length(x1),1); ones(length(x2),1)];
   XX=[X,X1,X2];
